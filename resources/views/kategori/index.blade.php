        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900">
                    ...
                    <div class="mb-6">
                        <form action="{{ route('kategori.index') }}" method="GET" class="flex flex-col sm:flex-row gap-3">
                            <x-text-input type="text" name="search" placeholder="Cari nama kategori..." class="flex-grow" value="{{ request('search') }}" />
                            <x-primary-button type="submit">
                                {{ __('Cari') }}
                            </x-primary-button>
                            @if(request('search'))
                            <a href="{{ route('kategori.index') }}" class="inline-flex items-center px-4 py-2 bg-gray-300 border border-transparent rounded-md font-semibold text-xs text-gray-800 uppercase tracking-widest hover:bg-gray-400 focus:bg-gray-400 active:bg-gray-900 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2 transition ease-in-out duration-150">
                                {{ __('Reset') }}
                            </a>
                            @endif
                        </form>
